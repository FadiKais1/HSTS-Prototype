...
public class ExamBuilderPage {
    ...
    private void initialize() {
        ...
        courseComboBox.valueProperty().addListener((observable, oldCourse, newCourse) -> {
            if (newCourse != null && isConfigured()) {
    ...
    private void handleNewExam() {
        ...
        updateTotal();
        setStatus("Complete the form and save the new DRAFT exam.");
        updateActionState();
    }
    ...
    private void showExam(ExamDTO loaded) {
        ...
        snapshots.sort(Comparator.comparingInt(ExamQuestionDTO::getOrderNumber));
        selectedQuestionItems.setAll(
                snapshots.stream().map(SelectedQuestionItem::fromSnapshot).toList()
        );
    ...
    private void updateActionState() {
        ...
        boolean editable = configured && isEditable() && !savePending && !submitPending;
        ...
        if (courseComboBox != null) {
            courseComboBox.setDisable(!editable || loadedExam != null || coursesLoading);
        }
        if (titleField != null) titleField.setDisable(!editable);
        if (durationField != null) durationField.setDisable(!editable);
        if (teacherNotesArea != null) teacherNotesArea.setDisable(!editable);
        if (studentInstructionsArea != null) studentInstructionsArea.setDisable(!editable);
        if (selectedQuestionsTable != null) selectedQuestionsTable.setEditable(editable);
        if (newButton != null) newButton.setDisable(!configured || savePending || submitPending);
        if (openButton != null) openButton.setDisable(!configured || examLoading
